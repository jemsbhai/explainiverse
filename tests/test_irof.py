# tests/test_irof.py
"""Contract tests for the one-dimensional local IROF adaptation.

The function groups adjacent numeric features, tracks one fixed output under
baseline replacement, and returns a normalized local AOC. Canonical IROF uses
image superpixels and dataset aggregation, so this score is not directly
comparable to published IROF values and is not a general quality verdict.
...
class TestIROFBasic:
    ...
    def test_returns_float(self, setup_model):
        ...
        score = compute_irof(adapter, instance, explanation, baseline="mean", background_data=X)

        assert isinstance(score, float)

    def test_coefficient_ordering_returns_float(self, setup_model):
        """A coefficient-derived ordering returns a numeric AOC."""
        adapter, X, y, feature_names = setup_model
        instance = X[0]

        # Use fitted coefficients as one deterministic ordering.
        model = adapter.model
    ...
    def test_deterministic_results(self, setup_model):
        ...
        score1 = compute_irof(adapter, instance, explanation, baseline="mean", background_data=X)

        score2 = compute_irof(adapter, instance, explanation, baseline="mean", background_data=X)
    ...
    def test_use_absolute_true(self, setup_model):
        ...
        score = compute_irof(
            adapter, instance, explanation, baseline="mean", background_data=X, use_absolute=True
        )
    ...
    def test_use_absolute_false(self, setup_model):
        ...
        score = compute_irof(
            adapter, instance, explanation, baseline="mean", background_data=X, use_absolute=False
        )
...
class TestSegmentSize:
    ...
    def test_default_segment_size(self, setup_model):
        ...
        result = compute_irof(
            adapter, instance, explanation, baseline="mean", background_data=X, return_details=True
        )
    ...
    def test_segment_size_exceeds_features_rejected(self, setup_model):
        """An impossible segment size is rejected rather than clamped."""
        adapter, X, y, feature_names = setup_model
        instance = X[0]
        ...
        with pytest.raises(ValueError, match="segment_size cannot exceed"):
            compute_irof(
                adapter,
                instance,
                explanation,
    ...
    def test_different_segment_sizes_different_scores(self, setup_model):
        ...
        score_seg1 = compute_irof(
            adapter, instance, explanation, baseline="mean", background_data=X, segment_size=1
        )

        score_seg2 = compute_irof(
            adapter, instance, explanation, baseline="mean", background_data=X, segment_size=2
        )
...
class TestReturnDetails:
    ...
    def test_aoc_matches_standalone(self, setup_model):
        ...
        standalone = compute_irof(
            adapter, instance, explanation, baseline="mean", background_data=X, return_details=False
        )
...
class TestIROFMultiSegment:
    ...
    def test_returns_dict(self, setup_model):
        ...
        result = compute_irof_multi_segment(
            adapter, instance, explanation, baseline="mean", background_data=X
        )
    ...
    def test_custom_segment_sizes(self, setup_model):
        ...
        result = compute_irof_multi_segment(
            adapter, instance, explanation, baseline="mean", background_data=X, segment_sizes=[1, 2]
        )
...
class TestBaselineTypes:
    ...
    def test_baseline_median(self, setup_model):
        ...
        score = compute_irof(adapter, instance, explanation, baseline="median", background_data=X)
    ...
    def test_baseline_scalar(self, setup_model):
        ...
        score = compute_irof(adapter, instance, explanation, baseline=0.0, background_data=X)
    ...
    def test_baseline_array(self, setup_model):
        ...
        score = compute_irof(adapter, instance, explanation, baseline=baseline_values)
    ...
    def test_baseline_callable(self, setup_model):
        ...
        score = compute_irof(
            adapter, instance, explanation, baseline=custom_baseline, background_data=X
        )
    ...
    def test_different_baselines_different_scores(self, setup_model):
        ...
        score_zero = compute_irof(adapter, instance, explanation, baseline=0.0, background_data=X)
...
class TestEdgeCases:
    ...
    def test_few_features(self, setup_iris):
        ...
        score = compute_irof(
            adapter, instance, explanation, baseline="mean", background_data=X_small
        )
...
class TestSemanticValidation:
    ...
    def test_two_attribution_orders_return_floats(self, setup_iris):
        """Two attribution orders both satisfy the numeric API contract."""
        X, y, feature_names = setup_iris
        ...
        # First deterministic ordering from model coefficients.
        true_importance = np.abs(model.coef_[0])
        good_exp = create_explanation(true_importance, feature_names)

        # Second seeded ordering; no quality comparison is asserted.
        np.random.seed(99)
        random_attrs = np.random.rand(4)
        random_exp = create_explanation(random_attrs, feature_names)

        good_score = compute_irof(adapter, instance, good_exp, baseline="mean", background_data=X)

        random_score = compute_irof(
            adapter, instance, random_exp, baseline="mean", background_data=X
        )

        assert isinstance(good_score, float)
        assert isinstance(random_score, float)

    def test_reversed_order_reports_reversed_segments(self, setup_iris):
        """Reported segment orders equal descending attribution-magnitude argsort."""
        X, y, feature_names = setup_iris
        ...
        assert good_result["segment_order"] == np.argsort(-true_importance).tolist()
        assert inverted_result["segment_order"] == np.argsort(-inverted_importance).tolist()

    def test_curve_has_one_point_per_removal_step(self, setup_model):
        """The reported curve contains its start plus one point per feature."""
        adapter, X, y, feature_names = setup_model
        instance = X[0]
        ...
        # This test checks only the documented curve structure.
...
class TestTargetClass:
    ...
    def test_explicit_targets_match_exact_normalised_output_curves(self, setup_iris):
        """Every IROF curve uses the exact explicitly selected probability column."""
        X, y, feature_names = setup_iris
        ...
        instance = X[0]
        attrs = np.array([0.4, 0.3, 0.2, 0.1])
        curve_inputs = [instance.copy()]
        current = instance.copy()
        for feature_index in [0, 1, 2, 3]:
            current = current.copy()
            current[feature_index] = 0.0
            curve_inputs.append(current)
        expected_probabilities = adapter.predict(np.asarray(curve_inputs))
        ...
        for target in [0, 1, 2]:
            ...
            np.testing.assert_allclose(result["curve"], expected_curve)
            assert result["original_prediction"] == pytest.approx(expected_predictions[0])
            assert result["aoc"] == pytest.approx(expected_aoc)
