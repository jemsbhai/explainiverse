...
"""Contract tests for the AIX360-style Monotonicity compatibility API.

The local binary fraction describes positive transitions along one feature-addition
path; it is not a general explanation-quality score.
...
class TestBaselineTypes:
    ...
    def test_scalar_baseline(self, setup_model):
        ...
        score = compute_monotonicity(data["adapter"], data["instance"], data["exp"], baseline=0.0)
...
class TestMultipleModels:
    ...
    def test_random_forest(self):
        ...
        score = compute_monotonicity(adapter, instance, exp, baseline="mean", background_data=X)
...
class TestSemanticValidation:
    ...
    def test_lime_scores_respect_output_range(self):
        """LIME integration returns values in the documented range."""
        # Create a simple fitted linear model.
        np.random.seed(42)
        n_samples = 100
    ...
    def test_lime_and_seeded_orderings_respect_output_range(self):
        """Two attribution sources satisfy the documented range contract."""
        iris = load_iris()
        X, y = iris.data, iris.target
        class_names = iris.target_names.tolist()
        feature_names = list(iris.feature_names)
        ...
        # Exercise two attribution sources without asserting relative quality.
        lime_scores = []
        random_scores = []
    ...
    def test_lime_and_negated_orderings_respect_output_range(self):
        """Both signed attribution orderings satisfy the range contract."""
        iris = load_iris()
        X, y = iris.data, iris.target
        class_names = iris.target_names.tolist()
        feature_names = list(iris.feature_names)
...
class TestTargetClass:
    ...
    def test_default_target_class(self):
        ...
        default_score = compute_monotonicity(
            adapter, instance, exp, baseline="mean", background_data=X
        )
        explicit_score = compute_monotonicity(
            adapter,
            instance,
            exp,
            baseline="mean",
