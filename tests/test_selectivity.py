...
"""Contract tests for the local numeric Selectivity/AOPC adaptation.

The function averages signed fixed-output drops along a caller-selected
baseline-replacement path. A larger value means a larger average drop under
that path; it is not a general ranking of explanation quality.
...
from explainiverse.evaluation import compute_batch_selectivity, compute_selectivity
...
class TestSelectivityBasic:
    ...
    def test_score_is_a_float(self, setup_model):
        """The signed local AOPC result is a float."""
        adapter, X, y, feature_names = setup_model
        instance = X[0]
...
class TestNSteps:
    ...
    def test_n_steps_exceeds_features_rejected(self, setup_model):
        """An impossible step count is rejected rather than clamped."""
        adapter, X, y, feature_names = setup_model
        instance = X[0]
        ...
        with pytest.raises(ValueError, match="n_steps cannot exceed"):
            compute_selectivity(
                adapter,
                instance,
                explanation,
    ...
    def test_n_steps_truncates_the_exact_perturbation_curve(self, setup_model):
        """Each n_steps value uses the corresponding AOPC curve prefix."""
        adapter, X, y, feature_names = setup_model
        instance = X[0]
        ...
        result_all = compute_selectivity(
            adapter,
            instance,
            explanation,
            baseline="mean",
        ...
        result_one = compute_selectivity(
            adapter,
            instance,
            explanation,
            baseline="mean",
        ...
        assert result_all["n_steps"] == 4
        assert result_one["n_steps"] == 1
        np.testing.assert_array_equal(result_one["feature_order"], result_all["feature_order"][:1])
        assert result_all["aopc"] == pytest.approx(np.mean(result_all["prediction_drops"]))
        assert result_one["aopc"] == pytest.approx(np.mean(result_one["prediction_drops"]))
...
class TestBaselineTypes:
    ...
    def test_baseline_median(self, setup_model):
        ...
        score = compute_selectivity(
            adapter, instance, explanation, baseline="median", background_data=X
        )
    ...
    def test_baseline_scalar(self, setup_model):
        ...
        score = compute_selectivity(adapter, instance, explanation, baseline=0.0, background_data=X)
    ...
    def test_baseline_array(self, setup_model):
        ...
        score = compute_selectivity(adapter, instance, explanation, baseline=baseline_values)
...
class TestSemanticValidation:
    ...
    def test_two_attribution_orders_return_floats(self, setup_iris):
        """Two attribution orderings both satisfy the numeric API contract."""
        X, y, feature_names = setup_iris
        ...
        # Use model coefficients as one arbitrary ordering.
        ...
        good_score = compute_selectivity(
            adapter, instance, good_exp, baseline="mean", background_data=X
        )

        # Use a second seeded ordering; no relative-quality claim is made.
        np.random.seed(99)
    ...
    def test_reversing_values_reverses_feature_order(self, setup_iris):
        """The reported perturbation order follows descending attribution."""
        X, y, feature_names = setup_iris
        ...
        normal_result = compute_selectivity(
            adapter,
            instance,
            normal_exp,
            baseline="mean",
        ...
        inverted_result = compute_selectivity(
            adapter,
            instance,
            inverted_exp,
            baseline="mean",
        ...
        assert normal_result["feature_order"].tolist() == [0, 1, 2, 3]
        assert inverted_result["feature_order"].tolist() == [3, 2, 1, 0]
...
class TestTargetClass:
    ...
    def test_explicit_target_columns_match_model_probabilities(self, setup_iris):
        """Every curve tracks the exact explicitly selected probability column."""
        X, y, feature_names = setup_iris
        ...
        perturbed_once = instance.copy()
        perturbed_once[0] = baseline[0]
        perturbed_twice = perturbed_once.copy()
        perturbed_twice[1] = baseline[1]
        curve_inputs = np.vstack([instance, perturbed_once, perturbed_twice])
        expected_probabilities = model.predict_proba(curve_inputs)
        ...
        for target in [0, 1, 2]:
            ...
            expected_curve = expected_probabilities[:, target]
            expected_drops = expected_curve[0] - expected_curve
            np.testing.assert_array_equal(result["feature_order"], [0, 1])
            np.testing.assert_allclose(result["predictions"], expected_curve)
            np.testing.assert_allclose(result["prediction_drops"], expected_drops)
            assert result["original_prediction"] == pytest.approx(expected_curve[0])
            assert result["aopc"] == pytest.approx(np.mean(expected_drops))
    ...
    def test_default_target_uses_explanation_identity(self, setup_model):
        ...
        explicit_score = compute_selectivity(
            adapter, instance, explanation, baseline="mean", background_data=X, target_class=0
        )
...
class TestComparisonWithPixelFlipping:
    ...
    def test_different_aggregation(self, setup_model):
        ...
        pf_result = compute_pixel_flipping(
            adapter, instance, explanation, baseline="mean", background_data=X, return_curve=True
        )
