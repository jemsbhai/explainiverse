...
def test_explicit_v0152_exception_requires_23_branch_controls_and_21_check_results():
    policy, _ = _policy()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("release_tag", "v0.15.3", "release_tag must be exactly"),
        ("package_version", "0.15.3", "package_version must be exactly"),
        ("merge_pull_request", 8, "merge_pull_request must be exactly"),
        ("merge_pull_request", 7.0, "merge_pull_request must be exactly"),
...
@pytest.mark.parametrize(
    ...
    [
        ("v0.15.2", "wrong-exception", "id must be exactly"),
        ("v0.15.3", CUDA_EXCEPTION_ID, "restricted to 'v0.15.2'"),
    ],
)
...
def test_exception_is_hard_bound_to_pr7_actual_merge_commit(field, replacement, match):
    policy, _ = _policy()
    observation = _matching_observation(CUDA_EXCEPTION_ID)
...
def test_capture_observation_requires_tag_absence_and_collects_detailed_ruleset(
    ...
    def get_json(path):
        if path == f"{root}/git/ref/tags/v0.15.2":
...
def test_exception_capture_reads_and_binds_pr7_merge_metadata():
    policy, _ = _policy()
    observation = _matching_observation(CUDA_EXCEPTION_ID)
    root = f"repos/{policy['repository']}"
    ...
    responses = {
        ...
        f"{root}/issues/7/timeline?per_page=100&page=1": [
            {
                "event": "merged",
    ...
    assert f"{root}/pulls/7" in requested
    assert f"{root}/issues/7/timeline?per_page=100&page=1" in requested
...
def test_exception_merge_event_reads_every_timeline_page():
    ...
    responses = {
        f"{root}/issues/7/timeline?per_page=100&page=1": first_page,
        f"{root}/issues/7/timeline?per_page=100&page=2": [],
    }
    requested = []
