...
SHA = "a" * 40
SOURCE_RUN_ID = "1234"
FILENAMES = ("explainiverse-0.15.2-py3-none-any.whl", "explainiverse-0.15.2.tar.gz")
...
def _artifacts(tmp_path):
    ...
    pypi = {
        "info": {"name": "explainiverse", "version": "0.15.2"},
        "urls": [
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("record", ("release", "tag"), "v0.15.3", "record tag"),
        ...
        ("run", ("head_branch",), "v0.15.3", "source run tag"),
...
@pytest.mark.parametrize(
    ("field", "replacement"),
    [
        ("exception_id", "EXPLAINIVERSE-v0.15.2-CPU-ONLY-forged"),
...
def test_original_dist_pypi_and_github_assets_must_have_identical_hashes(tmp_path):
    ...
    recovery.verify_pypi_json(pypi, project="explainiverse", version="0.15.2", expected=expected)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda value: value["info"].update(version="0.15.3"), "version mismatch"),
        (
...
def test_pypi_verification_fails_closed_on_identity_or_hash_drift(tmp_path, mutation, match):
    ...
    with pytest.raises(ValueError, match=match):
        recovery.verify_pypi_json(
            pypi, project="explainiverse", version="0.15.2", expected=expected
        )
