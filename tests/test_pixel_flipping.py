...
"""Contract tests for the numeric-vector Pixel-Flipping adaptation.

Pixel Flipping sequentially removes features in order of attributed importance
(highest selected attribution first) and integrates the tracked raw-output
curve. A lower AUC means faster degradation only under that output/baseline
contract; it is not a general faithfulness verdict or a published image score.
...
class TestCurveOutput:
    ...
    def test_curve_starts_at_original_output(self, setup_model):
        """The published procedure records raw model outputs."""
        data = setup_model
        ...
        assert result["curve"][0] == pytest.approx(result["predictions"][0])
...
class TestBaselineTypes:
    ...
    def test_scalar_baseline(self, setup_model):
        ...
        score = compute_pixel_flipping(data["adapter"], data["instance"], data["exp"], baseline=0.0)
...
class TestMultipleModels:
    ...
    def test_random_forest(self):
        ...
        score = compute_pixel_flipping(adapter, instance, exp, baseline="mean", background_data=X)
...
class TestSemanticValidation:
    ...
    def test_curve_starts_at_recorded_original_output(self):
        """The first curve point equals the stored unperturbed output."""
        iris = load_iris()
        X, y = iris.data, iris.target
        class_names = iris.target_names.tolist()
        feature_names = list(iris.feature_names)
        ...
        # First point is the original raw target output.
        assert curve[0] == pytest.approx(result["predictions"][0])
...
class TestTargetClass:
    ...
    def test_default_target_class(self):
        ...
        default_score = compute_pixel_flipping(
            adapter, instance, exp, baseline="mean", background_data=X
        )
        explicit_score = compute_pixel_flipping(
            adapter,
            instance,
            exp,
            baseline="mean",
