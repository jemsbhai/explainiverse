...
class TestExplanationFeatureNames:
    ...
    def test_explanation_get_top_features(self):
        ...
        exp = Explanation(
            explainer_name="Test",
            target_class="class_0",
            explanation_data={"feature_attributions": {"a": 0.1, "b": -0.8, "c": 0.5}},
        )
    ...
    def test_explanation_to_dict_and_from_dict(self):
        """Explanation dictionary conversion round-trips correctly."""
        from explainiverse.core.explanation import Explanation
...
class TestPyTorchAdapterBinaryClassification:
    ...
    def binary_classifier(self):
        ...
        model = nn.Sequential(nn.Linear(5, 10), nn.ReLU(), nn.Linear(10, 1))  # Single output
        return model
    ...
    def multiclass_classifier(self):
        ...
        model = nn.Sequential(nn.Linear(5, 10), nn.ReLU(), nn.Linear(10, 3))  # 3 classes
        return model
    ...
    def test_binary_classifier_predict(self, binary_classifier):
        ...
        adapter = PyTorchAdapter(
            binary_classifier, task="classification", class_names=["negative", "positive"]
        )
        ...
        np.testing.assert_allclose(preds.sum(axis=1), np.ones(5), atol=1e-6)
    ...
    def test_binary_classifier_gradients(self, binary_classifier):
        ...
        np.testing.assert_allclose(preds.sum(axis=1), np.ones(1), atol=1e-6)
    ...
    def test_multiclass_classifier_predict(self, multiclass_classifier):
        ...
        adapter = PyTorchAdapter(
            multiclass_classifier, task="classification", class_names=["a", "b", "c"]
        )
...
class TestIntegratedGradientsShapePreservation:
    ...
    def test_tabular_shape_preserved(self, tabular_model):
        ...
        explainer = IntegratedGradientsExplainer(
            model=adapter, feature_names=feature_names, n_steps=10
        )
    ...
    def test_image_shape_preserved(self, cnn_model):
        ...
        explainer = IntegratedGradientsExplainer(model=adapter, n_steps=5)
...
class TestExplanationSuiteRegistry:
    ...
    def test_suite_lazy_loads_registry(self):
        ...
        suite = ExplanationSuite(model=None, explainer_configs=[("lime", {})])
    ...
    def test_suite_list_methods(self):
        ...
        suite = ExplanationSuite(model=None, explainer_configs=[("lime", {}), ("shap", {})])
