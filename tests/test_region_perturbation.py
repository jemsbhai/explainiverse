...
"""Contract tests for a one-dimensional contiguous-group perturbation diagnostic.
...
The local relative-output AUC describes degradation under the selected grouping
and baseline. It is not the canonical Samek image procedure and does not by
itself rank explanation quality.
...
- Integration smoke tests for several attribution sources
...
class TestRegionPerturbationBasic:
    ...
    def test_probability_output_scores_are_nonnegative(self, setup_iris):
        """Probability-output curves have non-negative AUC on this fixture."""
        data = setup_iris
        ...
        # Every curve ordinate is a class probability on this fixture.
        for score in scores:
            assert score >= 0
...
class TestRegionSize:
    ...
    def setup_model(self):
        ...
        X, y = make_classification(
            n_samples=100, n_features=12, n_informative=6, n_classes=2, random_state=42
        )
        ...
        return {"X": X, "adapter": adapter, "instance": instance, "exp": exp, "n_features": 12}
    ...
    def test_region_size_large_rejected(self, setup_model):
        """An impossible region size is rejected rather than clamped."""
        data = setup_model

        with pytest.raises(ValueError, match="region_size cannot exceed"):
            compute_region_perturbation(
                data["adapter"],
                data["instance"],
                data["exp"],
    ...
    def test_region_sizes_report_expected_region_counts(self, setup_model):
        """Region count is the ceiling of feature count divided by region size."""
        data = setup_model
        ...
        for region_size in [1, 2, 3, 4, 6]:
            ...
            assert result["n_regions"] == int(np.ceil(12 / region_size))
...
class TestCurveOutput:
    ...
    def setup_model(self):
        ...
        return {"X": X, "adapter": adapter, "instance": instance, "exp": exp, "n_features": 8}
...
class TestEdgeCases:
    ...
    def test_few_features(self):
        ...
        X, y = make_classification(
            n_samples=50, n_features=3, n_informative=2, n_redundant=0, n_classes=2, random_state=42
        )
        class_names = ["class_0", "class_1"]
    ...
    def test_identical_attributions(self):
        ...
        X, y = make_classification(
            n_samples=50, n_features=8, n_informative=4, n_redundant=0, random_state=42
        )
        model = LogisticRegression(max_iter=200, random_state=42)
        model.fit(X, y)
    ...
    def test_odd_feature_count(self):
        ...
        X, y = make_classification(
            n_samples=50, n_features=7, n_informative=3, n_redundant=0, n_classes=2, random_state=42
        )
        class_names = ["class_0", "class_1"]
...
class TestSemanticValidation:
    ...
    def test_lime_and_seeded_attributions_return_finite_auc(self):
        """LIME and a seeded attribution vector both satisfy the API contract."""
        iris = load_iris()
        X, y = iris.data, iris.target
        class_names = iris.target_names.tolist()
        feature_names = list(iris.feature_names)
        ...
        assert all(np.isfinite(s) for s in lime_scores)
        assert all(np.isfinite(s) for s in random_scores)

    def test_lime_and_negated_attributions_return_finite_auc(self):
        """Both signed attribution orderings return finite AUC values."""
        iris = load_iris()
        X, y = iris.data, iris.target
        class_names = iris.target_names.tolist()
        feature_names = list(iris.feature_names)
        ...
        inverted_score = compute_region_perturbation(
            adapter, instance, inverted_exp, baseline="mean", background_data=X
        )

        print(f"\nLIME AUC: {lime_score:.4f}")
        print(f"Inverted AUC: {inverted_score:.4f}")

        assert np.isfinite(lime_score)
        assert np.isfinite(inverted_score)

    def test_curve_starts_at_normalized_original_output(self):
        """The normalized curve begins at one by construction."""
        iris = load_iris()
        X, y = iris.data, iris.target
        class_names = iris.target_names.tolist()
        feature_names = list(iris.feature_names)
        ...
        result = compute_region_perturbation(
            adapter, instance, exp, baseline="mean", background_data=X, return_curve=True
        )
...
class TestTargetClass:
    ...
    def test_explicit_target_class(self):
        ...
        target_class = class_names.index(exp.target_class)
        score = compute_region_perturbation(
            adapter,
            instance,
            exp,
        ...
        with pytest.raises(ValueError, match="conflicting target output metadata"):
            compute_region_perturbation(
                adapter,
                instance,
                exp,
                baseline="mean",
                background_data=X,
                target_class=(target_class + 1) % len(class_names),
            )

    def test_default_target_class(self):
        """The default target is the output identified by the explanation."""
        iris = load_iris()
        X, y = iris.data, iris.target
        class_names = iris.target_names.tolist()
        feature_names = list(iris.feature_names)
        ...
        default_score = compute_region_perturbation(
            adapter, instance, exp, baseline="mean", background_data=X
        )
        explicit_score = compute_region_perturbation(
            adapter,
            instance,
            exp,
            baseline="mean",
...
class TestComparisonWithPixelFlipping:
    """Smoke tests for the separate region-size contracts."""

    def test_larger_regions_vs_smaller(self):
        """Several valid region sizes each return a numeric score."""
        iris = load_iris()
        X, y = iris.data, iris.target
        class_names = iris.target_names.tolist()
        feature_names = list(iris.feature_names)
        ...
        for region_size in [1, 2, 4]:
            score = compute_region_perturbation(
                adapter, instance, exp, baseline="mean", background_data=X, region_size=region_size
            )
