...
"""
Tests for Grad-CAM, including explicit rejection of unsupported Grad-CAM++.
...
class TestGradCAMBasic:
    ...
    def test_gradcam_rejects_non_gradient_model(self, class_names):
        ...
        with pytest.raises(TypeError, match="get_layer_gradients"):
            GradCAMExplainer(model=adapter, target_layer="some_layer", class_names=class_names)
    ...
    def test_gradcam_explain(self, simple_cnn, sample_image, class_names):
        ...
        layers = adapter.list_layers()
        conv_layer = [layer for layer in layers if "conv" in layer][-1]  # Last conv layer
...
class TestGradCAMPlusPlusUnavailable:
    """The first-derivative adapter cannot establish general Grad-CAM++."""

    def test_gradcampp_is_not_mislabeled(self, simple_cnn, class_names):
        from explainiverse.adapters import PyTorchAdapter
        from explainiverse.explainers import GradCAMExplainer
...
class TestGradCAMTargetClass:
    ...
    def test_gradcam_target_class(self, simple_cnn, sample_image, class_names):
        ...
        # Explicit targets retain their exact labels and the shared spatial contract.
...
class TestGradCAMInputFormats:
    ...
    def test_gradcam_rejects_ambiguous_layout_unless_explicit(self):
        """Small RGB images require an explicit layout when both edges look like channels."""
        from explainiverse.adapters import PyTorchAdapter
        from explainiverse.explainers import GradCAMExplainer
        ...
        class SmallRGBNet(nn.Module):
            ...
            def forward(self, inputs):
                return self.head(self.conv(inputs).mean(dim=(2, 3)))
        ...
        explanation = GradCAMExplainer(adapter, "conv", input_layout="hwc").explain(
            image_hwc, target_class=0
        )
        assert explanation.explanation_data["input_shape"] == [1, 3, 4, 4]
        assert explanation.explanation_data["input_layout"] == "hwc"

        image_chw = np.transpose(image_hwc, (2, 0, 1))
        explanation = GradCAMExplainer(adapter, "conv", input_layout="chw").explain(
            image_chw, target_class=0
        )
        assert explanation.explanation_data["input_shape"] == [1, 3, 4, 4]
...
class TestGradCAMRegistry:
    ...
    def test_gradcam_via_registry(self, simple_cnn, class_names):
        ...
        explainer = default_registry.create(
            "gradcam", model=adapter, target_layer=conv_layer, class_names=class_names
        )
