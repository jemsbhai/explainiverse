...
with respect to the input. These tests make no comparative runtime or
explanation-quality claim.
...
class TestSaliencyBasic:
    ...
    def test_saliency_creation(self, simple_classifier, feature_names, class_names):
        ...
        assert explainer.absolute_value is True  # default
    ...
    def test_saliency_custom_parameters(self, simple_classifier, feature_names, class_names):
        ...
        assert explainer.absolute_value is False
    ...
    def test_saliency_rejects_non_gradient_model(self, feature_names):
        ...
        with pytest.raises(TypeError, match="predict_with_gradients"):
            SaliencyExplainer(model=adapter, feature_names=feature_names)
...
class TestSaliencyClassification:
    ...
    def test_saliency_explain_classification(
    ...
    def test_saliency_target_class(self, feature_names, class_names):
        """Each selected linear output returns its analytical derivative."""
        from explainiverse.adapters import PyTorchAdapter
        from explainiverse.explainers.gradient import SaliencyExplainer
    ...
    def test_saliency_auto_target_class(
        self, simple_classifier, sample_data, feature_names, class_names
    ):
    ...
    def test_saliency_attribution_shape(
        self, simple_classifier, sample_data, feature_names, class_names
    ):
    ...
    def test_saliency_default_absolute_value(
        self, simple_classifier, sample_data, feature_names, class_names
    ):
        ...
        explainer = SaliencyExplainer(
            model=adapter, feature_names=feature_names, class_names=class_names, absolute_value=True
        )
...
class TestSaliencyRegression:
    ...
    def test_saliency_explain_regression(self, simple_regressor, sample_data, feature_names):
        ...
        explainer = SaliencyExplainer(model=adapter, feature_names=feature_names)
    ...
    def test_saliency_regression_attributions_finite(
...
class TestSaliencyVariants:
    ...
    def test_saliency_signed(self, linear_model, sample_data, feature_names, class_names):
        """Signed saliency is exactly the selected linear-output gradient."""
        from explainiverse.adapters import PyTorchAdapter
        from explainiverse.explainers.gradient import SaliencyExplainer
        ...
        explanation = explainer.explain(sample_data[0], target_class=1)
        np.testing.assert_allclose(
            explanation.explanation_data["attributions_raw"],
            linear_model.weight[1].detach().numpy(),
            atol=1e-7,
        )
    ...
    def test_saliency_and_input_times_gradient_exact_relation(
        self, linear_model, sample_data, feature_names, class_names
    ):
        """Input×Gradient is the exact elementwise input-scaled gradient."""
        from explainiverse.adapters import PyTorchAdapter
        from explainiverse.explainers.gradient import SaliencyExplainer
        ...
        explanation_saliency = explainer.explain(sample_data[0], target_class=1, method="saliency")
        explanation_itg = explainer.explain(
            sample_data[0], target_class=1, method="input_times_gradient"
        )
        ...
        np.testing.assert_allclose(attr_itg, sample_data[0] * np.asarray(attr_saliency), atol=1e-7)
...
class TestSaliencyGradientCorrectness:
    ...
    def test_linear_model_gradient_equals_weights(
        self, linear_model, sample_data, feature_names, class_names
    ):
    ...
    def test_gradient_independent_of_input_for_linear(
        self, linear_model, sample_data, feature_names, class_names
    ):
...
class TestSaliencyDeterminism:
    """Tests repeated-call determinism for a fixed model and target."""

    def test_saliency_is_deterministic(
        self, simple_classifier, sample_data, feature_names, class_names
    ):
...
class TestSaliencyRegistry:
    ...
    def test_saliency_via_registry(self, simple_classifier, feature_names, class_names):
        ...
        explainer = default_registry.create(
            "saliency", model=adapter, feature_names=feature_names, class_names=class_names
        )
...
class TestSaliencyEdgeCases:
    ...
    def test_input_times_gradient_with_zero_input(
