...
"""Contract tests for the local Monotonicity-Nguyen proxy.
...
The implementation uses one selected baseline rather than the source
distributional expectation, so its score is a scoped local diagnostic.
...
class TestEdgeCases:
    ...
    def test_identical_attributions(self):
        ...
        with pytest.raises(ValueError, match="undefined for a constant input"):
            compute_monotonicity_nguyen(adapter, instance, exp, baseline="mean", background_data=X)
...
class TestSemanticValidation:
    ...
    def test_lime_and_reversed_orderings_respect_output_range(self):
        """Both attribution orderings satisfy the range contract."""
        iris = load_iris()
        X, y = iris.data, iris.target
        class_names = iris.target_names.tolist()
        feature_names = list(iris.feature_names)
...
class TestComparisonWithArya:
    ...
    def test_different_ranges(self):
        ...
        for i in range(20):
            ...
            arya_scores.append(
                compute_monotonicity(adapter, instance, exp, baseline="mean", background_data=X)
            )

            nguyen_scores.append(
                compute_monotonicity_nguyen(
                    adapter, instance, exp, baseline="mean", background_data=X
                )
            )
...
class TestTargetClass:
    ...
    def test_default_target_class(self):
        ...
        default_score = compute_monotonicity_nguyen(
            adapter, instance, exp, baseline="mean", background_data=X
        )
        explicit_score = compute_monotonicity_nguyen(
            adapter,
            instance,
            exp,
            baseline="mean",
