...
"""
Legacy integration tests for PGI, PGU, and related utilities.
...
from explainiverse.evaluation._utils import apply_feature_mask, compute_baseline_values, resolve_k
...
class TestPGI:
    ...
    def test_pgi_basic(self, setup_model_and_data):
        ...
        pgi = compute_pgi(
            data["adapter"], instance, exp, k=2, baseline="mean", background_data=data["X"]
        )
    ...
    def test_pgi_with_fraction_k(self, setup_model_and_data):
        ...
        pgi = compute_pgi(
            data["adapter"], instance, exp, k=0.5, baseline="mean", background_data=data["X"]
        )
    ...
    def test_pgi_increases_with_k(self, setup_model_and_data):
        ...
        pgi_1 = compute_pgi(
            data["adapter"], instance, exp, k=1, baseline="mean", background_data=data["X"]
        )
        pgi_2 = compute_pgi(
            data["adapter"], instance, exp, k=2, baseline="mean", background_data=data["X"]
        )
        pgi_3 = compute_pgi(
            data["adapter"], instance, exp, k=3, baseline="mean", background_data=data["X"]
        )
...
class TestFaithfulnessScore:
    ...
    def test_faithfulness_score_structure(self, setup_model_and_data):
        ...
        scores = compute_faithfulness_score(
            data["adapter"], instance, exp, k=0.2, baseline="mean", background_data=data["X"]
        )
...
class TestBatchAndComparison:
    ...
    def test_compare_explainer_faithfulness(self, setup_model_and_data):
        ...
        comparison_df = compare_explainer_faithfulness(
            ...
            k=0.2,
            baseline="mean",
            background_data=data["X"][n_samples:],
        )
