...
"""
Tests for attribution concentration and support-size diagnostics.
...
- Attribution Threshold Count — active-attribution count (legacy API name retained)
...
class _GradientProportionalExplainer:
    ...
    def explain(self, instance):
        ...
        attrs = {
            fn: float(self.scale * abs(instance[i])) for i, fn in enumerate(self.feature_names)
        }
        exp = Explanation(
...
class TestComputeSparseness:
    ...
    def test_range_zero_to_one(self, feature_names, single_instance):
        ...
        for Explainer in [
            _SparseExplainer,
            _UniformExplainer,
            _GradientProportionalExplainer,
            _TwoFeatureExplainer,
        ]:
    ...
    def test_ordering_sparse_gt_partial_gt_uniform(self, feature_names, single_instance):
        """
        Semantic ordering: perfectly sparse > two-feature > uniform.
        """
        from explainiverse.evaluation import compute_sparseness

        sparse_score = compute_sparseness(_SparseExplainer(feature_names), single_instance)
        partial_score = compute_sparseness(_TwoFeatureExplainer(feature_names), single_instance)
        uniform_score = compute_sparseness(_UniformExplainer(feature_names), single_instance)
        ...
        class _FixedExplainer:
            ...
            def explain(self, instance):
                exp = Explanation(
                    ...
                    target_class="class_0",
                    explanation_data={"feature_attributions": {"a": 1.0, "b": 2.0, "c": 3.0}},
                )
...
class TestComputeBatchSparseness:
    ...
    def test_max_instances_limits_evaluation(self, feature_names, sample_data):
        ...
        explainer = _UniformExplainer(feature_names)
        result = compute_batch_sparseness(explainer, sample_data, max_instances=5)
    ...
    def test_with_real_explainer(self, trained_model_and_explainer):
        ...
        _, explainer, X = trained_model_and_explainer
        result = compute_batch_sparseness(explainer, X[:10], max_instances=10)
...
class TestComputeComplexity:
    ...
    def test_uniform_attribution_is_ln_n(self, feature_names, single_instance):
        """
        Perfectly uniform attributions over n features:
        H = ln(n). For n=4, expected = ln(4).
        """
        from explainiverse.evaluation import compute_complexity

        explainer = _UniformExplainer(feature_names, value=1.0)
        result = compute_complexity(explainer, single_instance)
        expected = np.log(len(feature_names))
        assert result == pytest.approx(expected, abs=1e-10)

    def test_uniform_8_features(self, feature_names_8):
        """
        Uniform over 8 features: H = ln(8).
        """
        from explainiverse.evaluation import compute_complexity
        ...
        assert result == pytest.approx(np.log(8), abs=1e-10)
    ...
    def test_range_zero_to_lnn(self, feature_names, single_instance):
        """Complexity should always be in [0, ln(n)]."""
        from explainiverse.evaluation import compute_complexity
        ...
        for Explainer in [
            ...
            result = compute_complexity(explainer, single_instance)
            assert (
                0.0 <= result <= max_entropy + 1e-10
            ), f"{Explainer.__name__} gave complexity {result} outside [0, {max_entropy}]"

    def test_zero_attributions_are_undefined(self, feature_names, single_instance):
        """All-zero attributions cannot define fractional contributions."""
        from explainiverse.evaluation import compute_complexity

        explainer = _ZeroExplainer(feature_names)
        with pytest.raises(ValueError, match="undefined for an all-zero"):
            compute_complexity(explainer, single_instance)
    ...
    def test_two_feature_known_entropy(self, feature_names, single_instance):
        ...
        H = -(0.7*ln(0.7) + 0.3*ln(0.3)) = 0.61086 nats
        """
        from explainiverse.evaluation import compute_complexity

        explainer = _TwoFeatureExplainer(feature_names, weight_a=0.7, weight_b=0.3)
        result = compute_complexity(explainer, single_instance)
        expected = -(0.7 * np.log(0.7) + 0.3 * np.log(0.3))
    ...
    def test_known_entropy_three_values(self):
        ...
        H = -(1/6*ln(1/6) + 1/3*ln(1/3) + 1/2*ln(1/2))
        """
        from explainiverse.evaluation import compute_complexity
    ...
    def test_inverse_relationship_to_sparseness(self, feature_names, single_instance):
        ...
        """
        from explainiverse.evaluation import compute_complexity, compute_sparseness
    ...
    def test_ordering_sparse_lt_partial_lt_uniform(self, feature_names, single_instance):
        """
        Entropy ordering for these controlled vectors:
        sparse < two-feature < uniform. No explanation-quality ordering is
        implied.
        """
        from explainiverse.evaluation import compute_complexity

        sparse_score = compute_complexity(_SparseExplainer(feature_names), single_instance)
        partial_score = compute_complexity(_TwoFeatureExplainer(feature_names), single_instance)
        uniform_score = compute_complexity(_UniformExplainer(feature_names), single_instance)
        assert sparse_score < partial_score < uniform_score
    ...
    def test_with_real_explainer(self, trained_model_and_explainer):
        ...
        assert isinstance(result, float)
        max_entropy = np.log(4)  # 4 features
...
class TestComputeBatchComplexity:
    ...
    def test_max_instances_limits_evaluation(self, feature_names, sample_data):
        ...
        explainer = _UniformExplainer(feature_names)
        result = compute_batch_complexity(explainer, sample_data, max_instances=5)
        assert result["n_evaluated"] == 5
        assert len(result["scores"]) == 5

    def test_uniform_batch_all_lnn(self, feature_names, sample_data):
        """Uniform explainer should yield all ln(n) complexity scores."""
        from explainiverse.evaluation import compute_batch_complexity
        ...
        expected = np.log(len(feature_names))
        assert result["mean"] == pytest.approx(expected, abs=1e-10)
        assert all(s == pytest.approx(expected, abs=1e-10) for s in result["scores"])
    ...
    def test_with_real_explainer(self, trained_model_and_explainer):
        ...
        _, explainer, X = trained_model_and_explainer
        result = compute_batch_complexity(explainer, X[:10], max_instances=10)
        assert result["n_evaluated"] == 10
...
# =============================================================================
# Legacy Effective Complexity Name — Attribution Threshold Count compatibility
# =============================================================================


class TestComputeEffectiveComplexity:
    """Compatibility tests for the historical threshold-count API name."""

    def test_import(self):
        """Historical compatibility function remains importable."""
    ...
    def test_unnormalized_range_zero_to_n(self, feature_names, single_instance):
        ...
        for Explainer in [
            ...
            assert 0.0 <= result <= n, f"{Explainer.__name__} gave EC {result} outside [0, {n}]"
    ...
    def test_ordering_sparse_lt_partial_lt_uniform(self, feature_names, single_instance):
        ...
        sparse_ec = compute_effective_complexity(
            _SparseExplainer(feature_names),
            single_instance,
            threshold=1e-5,
            threshold_type="absolute",
        )
        partial_ec = compute_effective_complexity(
            _TwoFeatureExplainer(feature_names),
            single_instance,
            threshold=1e-5,
            threshold_type="absolute",
        )
        uniform_ec = compute_effective_complexity(
            _UniformExplainer(feature_names),
            single_instance,
            threshold=1e-5,
            threshold_type="absolute",
        )
