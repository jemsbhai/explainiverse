...
TCAV reports the fraction of selected-output directional derivatives that are
positive along learned concept activation vectors. The statistic does not by
itself establish causal concept influence or human interpretability.
...
def assert_python_float(value, name: str):
    ...
    assert type(value) is float, (
        f"{name} should be Python float, got {type(value).__name__} " f"(value: {value})"
    )
...
def assert_python_int(value, name: str):
    ...
    assert type(value) is int, (
        f"{name} should be Python int, got {type(value).__name__} " f"(value: {value})"
    )
...
def assert_python_bool(value, name: str):
    ...
    assert type(value) is bool, (
        f"{name} should be Python bool, got {type(value).__name__} " f"(value: {value})"
    )
...
def assert_python_list(value, name: str):
    ...
    Args:
        ...
        name: Descriptive name for error messages
    """
    assert type(value) is list, f"{name} should be Python list, got {type(value).__name__}"
...
def assert_python_str(value, name: str):
    ...
    Args:
        ...
        name: Descriptive name for error messages
    """
    assert type(value) is str, f"{name} should be Python str, got {type(value).__name__}"
...
def simple_classifier():
    ...
    model = nn.Sequential(
        nn.Linear(8, 32), nn.ReLU(), nn.Linear(32, 16), nn.ReLU(), nn.Linear(16, 3)
    )
...
def concept_data():
    ...
    return {"concept_a": concept_a, "concept_b": concept_b, "random": random_examples}
...
class TestTCAVReturnTypes:
    ...
    def test_cav_accuracy_is_python_float(self, deeper_network, concept_data, class_names):
        ...
        explainer = TCAVExplainer(model=adapter, layer_name="layer2", class_names=class_names)
    ...
    def test_compute_tcav_score_returns_python_float(
        self, deeper_network, concept_data, sample_data, class_names
    ):
        ...
        tcav_score = explainer.compute_tcav_score(
            test_inputs=sample_data, target_class=0, concept_name="concept_a"
        )

        assert_python_float(tcav_score, "tcav_score")

    def test_compute_tcav_score_with_derivatives_returns_python_float(
        self, deeper_network, concept_data, sample_data, class_names
    ):
    ...
    def test_statistical_significance_test_all_types(
        self, deeper_network, concept_data, sample_data, class_names
    ):
    ...
    def test_get_most_influential_concepts_returns_python_floats(
        self, deeper_network, concept_data, sample_data, class_names
    ):
        ...
        top_concepts = explainer.get_most_influential_concepts(
            test_inputs=sample_data, target_class=0, top_k=2
        )
    ...
    def test_compare_concepts_returns_python_floats(
        self, deeper_network, concept_data, sample_data, class_names
    ):
        ...
        comparison = explainer.compare_concepts(test_inputs=sample_data, target_classes=[0, 1, 2])

        for concept_name, class_scores in comparison.items():
            assert_python_str(concept_name, "concept name")
    ...
    def test_explain_tcav_scores_types(
        self, deeper_network, concept_data, sample_data, class_names
    ):
        ...
        explanation = explainer.explain(test_inputs=sample_data, target_class=0)
    ...
    def test_explain_with_significance_test_types(
        self, deeper_network, concept_data, sample_data, class_names
    ):
        ...
        for concept_name, result in sig_tests.items():
            ...
            assert_python_list(
                result["random_scores"], f"sig_test random_scores for {concept_name}"
            )
    ...
    def test_random_cav_metadata_types(self, deeper_network, concept_data, class_names):
        ...
        random_cavs = explainer.learn_random_concepts(
            negative_examples=concept_data["random"], n_random=3
        )

        for i, cav in enumerate(random_cavs):
            assert_python_float(cav.accuracy, f"random_cav[{i}].accuracy")
            assert_python_int(
                cav.metadata["random_seed"], f"random_cav[{i}].metadata['random_seed']"
            )
...
class TestTCAVBasic:
    ...
    def test_tcav_creation(self, simple_classifier, class_names):
        ...
        explainer = TCAVExplainer(model=adapter, layer_name=layer_name, class_names=class_names)
    ...
    def test_tcav_invalid_layer(self, simple_classifier, class_names):
        ...
        with pytest.raises(ValueError, match="not found"):
            TCAVExplainer(model=adapter, layer_name="nonexistent_layer", class_names=class_names)
    ...
    def test_tcav_rejects_non_layer_model(self, class_names):
        ...
        with pytest.raises(TypeError, match="get_layer_output"):
            TCAVExplainer(model=adapter, layer_name="some_layer", class_names=class_names)
    ...
    def test_tcav_custom_classifier(self, deeper_network, class_names):
        ...
        explainer_logistic = TCAVExplainer(
            model=adapter, layer_name="layer2", class_names=class_names, cav_classifier="logistic"
        )
        assert explainer_logistic.cav_classifier == "logistic"

        # Test SGD
        explainer_sgd = TCAVExplainer(
            model=adapter, layer_name="layer2", class_names=class_names, cav_classifier="sgd"
        )
...
class TestTCAVConceptLearning:
    ...
    def test_learn_concept_basic(self, deeper_network, concept_data, class_names):
        ...
        from explainiverse.adapters import PyTorchAdapter
        from explainiverse.explainers.gradient import ConceptActivationVector, TCAVExplainer
    ...
    def test_cav_low_accuracy_raises_error(self, simple_classifier, class_names):
        ...
        explainer = TCAVExplainer(
            model=adapter, layer_name=layers[1], class_names=class_names  # Use a layer
        )
    ...
    def test_learn_random_concepts(self, deeper_network, concept_data, class_names):
        ...
        random_cavs = explainer.learn_random_concepts(
            negative_examples=concept_data["random"], n_random=5
        )
...
class TestTCAVScoreComputation:
    ...
    def test_tcav_score_returns_derivatives(
        self, deeper_network, concept_data, sample_data, class_names
    ):
    ...
    def test_tcav_score_matches_derivative_sign_fraction_for_each_class(
        self, deeper_network, concept_data, sample_data, class_names
    ):
        """Each target's score is exactly its positive-derivative fraction."""
        from explainiverse.adapters import PyTorchAdapter
        from explainiverse.explainers.gradient import TCAVExplainer
        ...
        for target_class in [0, 1]:
            score, derivatives = explainer.compute_tcav_score(
                test_inputs=sample_data,
            ...
            assert_python_float(score, f"score_class_{target_class}")
            assert len(derivatives) == len(sample_data)
            assert score == float(np.mean(derivatives > 0))

    def test_tcav_score_unknown_concept_raises_error(
        self, deeper_network, sample_data, class_names
    ):
        ...
        with pytest.raises(ValueError, match="not found"):
            explainer.compute_tcav_score(
                test_inputs=sample_data, target_class=0, concept_name="unknown_concept"
            )
    ...
    def test_directional_derivative(self, deeper_network, concept_data, sample_data, class_names):
        ...
        derivatives = explainer.compute_directional_derivative(
            inputs=sample_data[:5], cav=cav, target_class=0
        )
...
class TestTCAVStatistics:
    ...
    def test_significance_with_custom_alpha(
        self, deeper_network, concept_data, sample_data, class_names
    ):
...
class TestTCAVExplain:
    ...
    def test_explain_multiple_concepts(
        self, deeper_network, concept_data, sample_data, class_names
    ):
    ...
    def test_explain_with_significance_test(
        self, deeper_network, concept_data, sample_data, class_names
    ):
    ...
    def test_explain_auto_target_class(
        self, deeper_network, concept_data, sample_data, class_names
    ):
        ...
        explanation = explainer.explain(test_inputs=sample_data, target_class=None)  # Auto-select
    ...
    def test_explain_selective_concepts(
        self, deeper_network, concept_data, sample_data, class_names
    ):
        ...
        explanation = explainer.explain(
            test_inputs=sample_data, target_class=0, concept_names=["concept_a"]
        )
...
class TestTCAVComparison:
    ...
    def test_get_most_influential_concepts(
        self, deeper_network, concept_data, sample_data, class_names
    ):
...
class TestTCAVRegistry:
    ...
    def test_tcav_via_registry(self, deeper_network, class_names):
        ...
        explainer = default_registry.create(
            "tcav", model=adapter, layer_name="layer2", class_names=class_names
        )
...
class TestTCAVEdgeCases:
    ...
    def test_single_test_input(self, deeper_network, concept_data, class_names):
        ...
        tcav_score = explainer.compute_tcav_score(
            test_inputs=single_input, target_class=0, concept_name="concept_a"
        )
    ...
    def test_large_batch(self, deeper_network, concept_data, class_names):
        ...
        tcav_score = explainer.compute_tcav_score(
            test_inputs=large_batch, target_class=0, concept_name="concept_a"
        )
    ...
    def test_deterministic_results(self, deeper_network, concept_data, sample_data, class_names):
        ...
        explainer1 = TCAVExplainer(
            model=adapter, layer_name="layer2", class_names=class_names, random_seed=42
        )
    ...
    def test_different_layer_produces_different_cav(
        self, deeper_network, concept_data, class_names
    ):
        ...
        explainer1 = TCAVExplainer(model=adapter, layer_name="layer1", class_names=class_names)

        explainer2 = TCAVExplainer(model=adapter, layer_name="layer2", class_names=class_names)
...
class TestTCAVCNN:
    ...
    def test_tcav_with_cnn(self, cnn_model, class_names):
        ...
        explainer = TCAVExplainer(model=adapter, layer_name="conv1", class_names=class_names)
        ...
        tcav_score = explainer.compute_tcav_score(
            test_inputs=test_images, target_class=0, concept_name="bright_top"
        )
