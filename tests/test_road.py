# tests/test_road.py
"""Contract tests for the local ROAD-inspired numeric diagnostic.

The implementation applies a local noisy-linear imputer and reports signed
fixed-output changes for one instance. It is not Rong et al.'s canonical
dataset-level accuracy curve and does not certify that perturbations are
in-distribution.

Two orderings:
- MoRF (Most Relevant First): descending attribution order
- LeRF (Least Relevant First): ascending attribution order
...
10. Edge cases
11. Formula and structure validation
...
class TestBasicFunctionality:
    ...
    def test_returns_float(self, setup_model):
        ...
        score = compute_road(model, instance, explanation, background_data=X, seed=42)
    ...
    def test_deterministic_with_seed(self, setup_model):
        ...
        score1 = compute_road(model, instance, explanation, background_data=X, seed=42)
        score2 = compute_road(model, instance, explanation, background_data=X, seed=42)

        assert score1 == score2

    def test_seed_is_offset_by_each_removal_percentage(self, setup_model, monkeypatch):
        """ROAD passes the documented base-seed offset to each imputation step."""
        import explainiverse.evaluation.faithfulness_extended as extended
        ...
        monkeypatch.setattr(extended, "_noisy_linear_impute", identity_imputer)
        ...
        assert observed_seeds == [242, 542]
        expected_output = model.predict_proba(instance.reshape(1, -1))[0, 0]
        assert result["original_prediction"] == pytest.approx(expected_output)
        np.testing.assert_allclose(result["predictions"], [expected_output, expected_output])
        np.testing.assert_array_equal(result["prediction_changes"], [0.0, 0.0])
        assert result["score"] == pytest.approx(0.0, abs=0.0)
    ...
    def test_background_data_required(self, setup_model):
        ...
        with pytest.raises(ValueError, match="background_data is required"):
            compute_road(model, instance, explanation, background_data=None, seed=42)
    ...
    def test_invalid_background_data_shape(self, setup_model):
        ...
        with pytest.raises(ValueError, match="must be 2D with"):
            compute_road(model, instance, explanation, background_data=bad_background, seed=42)
    ...
    def test_custom_percentages(self, setup_model):
        ...
        score = compute_road(
            model, instance, explanation, background_data=X, percentages=custom_percentages, seed=42
        )
...
class TestOrderings:
    ...
    def test_morf_order(self, setup_model):
        ...
        score = compute_road(model, instance, explanation, background_data=X, order="morf", seed=42)
    ...
    def test_lerf_order(self, setup_model):
        ...
        score = compute_road(model, instance, explanation, background_data=X, order="lerf", seed=42)

        assert isinstance(score, float)

    def test_morf_lerf_return_float_scores(self, setup_model):
        """Both ordering modes return numeric scores."""
        model, X, y = setup_model
        instance = X[0]
        ...
        score_morf = compute_road(
            model, instance, explanation, background_data=X, order="morf", seed=42
        )
        score_lerf = compute_road(
            model, instance, explanation, background_data=X, order="lerf", seed=42
        )
    ...
    def test_invalid_order_raises(self, setup_model):
        ...
        with pytest.raises(ValueError, match="order must be"):
            compute_road(model, instance, explanation, background_data=X, order="invalid", seed=42)
    ...
    def test_default_order_is_morf(self, setup_model):
        ...
        score_default = compute_road(model, instance, explanation, background_data=X, seed=42)
        score_morf = compute_road(
            model, instance, explanation, background_data=X, order="morf", seed=42
        )
    ...
    def test_attribution_vectors_define_exact_morf_orders_and_score_equation(self, setup_model):
        """Each attribution vector fixes the stable MoRF order and mean-drop score."""
        model, X, y = setup_model
        instance = X[0]

        # First vector derived from model coefficients.
        ...
        # Second seeded vector; no relative-quality claim is asserted.
        np.random.seed(999)
        random_attributions = np.abs(np.random.randn(len(instance)))
        random_explanation = create_explanation(random_attributions)

        for attribution_values, explanation in [
            (good_attributions, good_explanation),
            (random_attributions, random_explanation),
        ]:
            result = compute_road(
                model,
                instance,
            ...
            expected_order = np.argsort(-np.abs(attribution_values), kind="stable")
            np.testing.assert_array_equal(result["feature_order"], expected_order)
            np.testing.assert_allclose(
                result["prediction_changes"],
                result["original_prediction"] - result["predictions"],
            )
            assert result["score"] == pytest.approx(np.mean(result["prediction_changes"]))
...
class TestCombinedVariant:
    ...
    def test_combined_returns_dict(self, setup_model):
        ...
        result = compute_road_combined(model, instance, explanation, background_data=X, seed=42)
    ...
    def test_combined_with_custom_percentages(self, setup_model):
        ...
        result = compute_road_combined(
            model, instance, explanation, background_data=X, percentages=[0.25, 0.5, 0.75], seed=42
        )
...
class TestBatchOperations:
    ...
    def test_batch_returns_dict(self, setup_batch):
        ...
        result = compute_batch_road(model, X[:10], explanations, background_data=X[10:], seed=42)
    ...
    def test_batch_max_samples_parameter(self, setup_batch):
        ...
        result = compute_batch_road(
            model, X[:10], explanations, background_data=X[10:], max_samples=5, seed=42
        )
    ...
    def test_batch_with_morf_order(self, setup_batch):
        ...
        result = compute_batch_road(
            model, X[:10], explanations, background_data=X[10:], order="morf", seed=42
        )
    ...
    def test_batch_with_lerf_order(self, setup_batch):
        ...
        result = compute_batch_road(
            model, X[:10], explanations, background_data=X[10:], order="lerf", seed=42
        )

        assert isinstance(result["mean"], float)
        assert not np.isnan(result["mean"])

    def test_batch_requires_explicit_background_data(self, setup_batch):
        """Batch ROAD never reuses evaluated rows as its background."""
        model, X, explanations = setup_batch

        with pytest.raises(ValueError, match="background_data is required"):
            compute_batch_road(model, X[:10], explanations, background_data=None, seed=42)
    ...
    def test_batch_custom_percentages(self, setup_batch):
        ...
        result = compute_batch_road(
            model, X[:10], explanations, background_data=X[10:], percentages=[0.3, 0.6], seed=42
        )
...
class TestBackgroundDataHandling:
    ...
    def test_background_data_used_for_imputation(self, setup_model):
        """Two valid background subsets each return a numeric result."""
        model, X, y = setup_model
        instance = X[0]
        ...
        score1 = compute_road(model, instance, explanation, background_data=background1, seed=42)
        score2 = compute_road(model, instance, explanation, background_data=background2, seed=42)
    ...
    def test_small_background_data(self, setup_model):
        ...
        score = compute_road(
            model, instance, explanation, background_data=small_background, seed=42
        )
    ...
    def test_large_background_data(self, setup_model):
        ...
        score = compute_road(
            model, instance, explanation, background_data=X, seed=42  # Full dataset
        )
    ...
    def test_1d_background_raises(self, setup_model):
        ...
        with pytest.raises(ValueError):
            compute_road(model, instance, explanation, background_data=X[0], seed=42)  # 1D
...
class TestPercentagesParameter:
    ...
    def test_invalid_percentages_rejected(self, setup_model):
        """Invalid percentages are rejected rather than silently removed."""
        model, X, y = setup_model
        instance = X[0]
...
class TestNoiseScaleParameter:
    ...
    def test_zero_noise_scale(self, setup_model):
        """A zero noise scale makes the result independent of the RNG seed."""
        model, X, y = setup_model
        instance = X[0]
        ...
        score1 = compute_road(
            model, instance, explanation, background_data=X, noise_scale=0.0, seed=42
        )
        score2 = compute_road(
            model,
            instance,
            ...
            background_data=X,
            noise_scale=0.0,
            seed=123,  # Zero-noise imputation is seed-independent.
        )

        assert score1 == pytest.approx(score2, rel=0.0, abs=0.0)
    ...
    def test_high_noise_scale(self, setup_model):
        ...
        score = compute_road(
            model, instance, explanation, background_data=X, noise_scale=5.0, seed=42
        )

        assert isinstance(score, float)

    def test_multiple_noise_scales_return_valid_results(self, setup_model):
        """Two non-negative noise scales each return a numeric result."""
        model, X, y = setup_model
        instance = X[0]
        ...
        score_low = compute_road(
            model, instance, explanation, background_data=X, noise_scale=0.1, seed=42
        )
        score_high = compute_road(
            model, instance, explanation, background_data=X, noise_scale=2.0, seed=42
        )
...
class TestEdgeCases:
    ...
    def test_few_features(self, setup_iris):
        ...
        score = compute_road(model, instance, explanation, background_data=X_small, seed=42)
...
class TestSemanticValidation:
    """Formula and result-structure checks for the local diagnostic."""

    def test_morf_returns_one_change_per_percentage(self, setup_model):
        """Detailed MoRF output contains one change per configured percentage."""
        model, X, y = setup_model
        instance = X[0]
        ...
        assert len(result["prediction_changes"]) == len(result["percentages"])
    ...
    def test_combined_gap_is_numeric_for_coefficient_order(self, setup_model):
        """The combined convenience gap is numeric for one attribution order."""
        model, X, y = setup_model
        instance = X[0]

        # Use model coefficients as one deterministic ordering.
        pred_class = model.predict(instance.reshape(1, -1))[0]
        coefs = model.coef_[pred_class]
        good_attributions = np.abs(coefs * instance)
        good_explanation = create_explanation(good_attributions)
        ...
        result = compute_road_combined(
            model, instance, good_explanation, background_data=X, seed=42
        )
...
class TestTargetClassHandling:
    ...
    def test_explicit_target_columns_match_model_outputs(self, setup_model, monkeypatch):
        """Each explicit target tracks that exact probability column throughout."""
        import explainiverse.evaluation.faithfulness_extended as extended

        model, X, y = setup_model
        instance = X[0]
        imputed_instance = X[1].copy()

        n_features = X.shape[1]
        attributions = np.random.randn(n_features)

        def fixed_imputer(*args, **kwargs):
            return imputed_instance.copy()

        monkeypatch.setattr(extended, "_noisy_linear_impute", fixed_imputer)
        original_probabilities = model.predict_proba(instance.reshape(1, -1))[0]
        imputed_probabilities = model.predict_proba(imputed_instance.reshape(1, -1))[0]
        ...
        for target in range(3):
            ...
            expected_drop = original_probabilities[target] - imputed_probabilities[target]
            assert result["original_prediction"] == pytest.approx(original_probabilities[target])
            np.testing.assert_allclose(
                result["predictions"], np.full(2, imputed_probabilities[target])
            )
            np.testing.assert_allclose(result["prediction_changes"], np.full(2, expected_drop))
            assert result["score"] == pytest.approx(expected_drop)
    ...
    def test_conflicting_explicit_target_is_rejected(self, setup_model):
        """The metric refuses an output index that conflicts with the explanation."""
        model, X, y = setup_model
        instance = X[0]

        n_features = X.shape[1]
        attributions = np.random.randn(n_features)
        explanation = create_explanation(attributions, target_class=0)

        with pytest.raises(ValueError, match="[Cc]onflicting target"):
            compute_road(
                model,
                instance,
                explanation,
...
class TestUseAbsoluteParameter:
    ...
    def test_default_use_absolute_true(self, setup_model):
        ...
        score_default = compute_road(model, instance, explanation, background_data=X, seed=42)
        score_explicit = compute_road(
            model, instance, explanation, background_data=X, use_absolute=True, seed=42
        )
    ...
    def test_use_absolute_false(self, setup_model):
        ...
        score = compute_road(
            model, instance, explanation, background_data=X, use_absolute=False, seed=42
        )
...
class TestNoisyLinearImpute:
    ...
    def test_basic_imputation(self):
        ...
        imputed = _noisy_linear_impute(
            instance, removed_indices, remaining_indices, background, noise_scale=1.0, seed=42
        )
    ...
    def test_zero_noise_deterministic(self):
        ...
        imputed1 = _noisy_linear_impute(
            instance, removed_indices, remaining_indices, background, noise_scale=0.0, seed=42
        )
        imputed2 = _noisy_linear_impute(
            instance,
            removed_indices,
...
class TestIntegration:
    ...
    def test_full_workflow(self, setup_model):
        ...
        combined = compute_road_combined(model, instance, explanation, background_data=X, seed=42)
