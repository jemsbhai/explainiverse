...
The implementation propagates relevance through explicitly supported PyTorch
graphs using configured rules. Conservation depends on the graph, rule, and
stabilization settings; only controlled analytical cases assert it exactly.

Propagation Rules:
- LRP-0: Basic rule (no stabilization)
- LRP-ε (epsilon): Adds small constant for numerical stability (default)
- LRP-γ (gamma): Applies the configured positive-weight modifier
- LRP-αβ (alpha-beta): Uses separate positive/negative terms
- LRP-z⁺ (z-plus): Uses positive components under the supported contract
...
class TestLRPBasic:
    ...
    def test_lrp_creation_z_plus_rule(self, simple_classifier, feature_names, class_names):
        ...
        explainer = LRPExplainer(
            model=adapter, feature_names=feature_names, class_names=class_names, rule="z_plus"
        )
    ...
    def test_lrp_rejects_non_pytorch_model(self, feature_names):
        ...
        with pytest.raises(TypeError, match="PyTorch"):
            LRPExplainer(model=adapter, feature_names=feature_names)
...
class TestLRPClassification:
    ...
    def test_lrp_explain_classification(
    ...
    def test_lrp_target_class(self, feature_names, class_names):
        """LRP-0 returns each selected linear output's exact contributions."""
        from explainiverse.adapters import PyTorchAdapter
        from explainiverse.explainers.gradient import LRPExplainer
        ...
        for target_class, weights_for_output in enumerate(weights[:2]):
            ...
            np.testing.assert_allclose(
                explanation.explanation_data["attributions_raw"], expected, atol=1e-6
            )
            assert explanation.target_class == class_names[target_class]

    def test_lrp_auto_target_class(
        self, simple_classifier, sample_data, feature_names, class_names
    ):
    ...
    def test_lrp_attribution_shape(
        self, simple_classifier, sample_data, feature_names, class_names
    ):
    ...
    def test_lrp_attributions_finite(
        self, simple_classifier, sample_data, feature_names, class_names
    ):
...
class TestLRPRegression:
    ...
    def test_lrp_explain_regression(self, simple_regressor, sample_data, feature_names):
        ...
        explainer = LRPExplainer(model=adapter, feature_names=feature_names)
...
# =============================================================================
# Conservation residual tests
# =============================================================================


class TestLRPConservation:
    """Tests for LRP's reported conservation residual.

    Exact equality is tested only for the epsilon-zero linear fixture below.
    Other rules expose the signed residual rather than receiving an arbitrary
    pass/fail quality threshold.
    """
    ...
    def test_epsilon_rule_reports_residual_identity(
        self, simple_classifier, sample_data, feature_names, class_names
    ):
        """The signed residual equals target output minus attribution sum."""
        from explainiverse.adapters import PyTorchAdapter
        from explainiverse.explainers.gradient import LRPExplainer
        ...
        explanation = explainer.explain(
            instance, target_class=target_class, return_convergence_delta=True
        )
        data = explanation.explanation_data
        expected_signed_delta = data["target_output"] - data["attribution_sum"]
        assert data["signed_convergence_delta"] == pytest.approx(expected_signed_delta)
        assert data["convergence_delta"] == pytest.approx(abs(expected_signed_delta))

    def test_conservation_return_delta(
        self, simple_classifier, sample_data, feature_names, class_names
    ):
        ...
        explanation = explainer.explain(
            sample_data[0], target_class=0, return_convergence_delta=True
        )
        ...
        data = explanation.explanation_data
        assert data["convergence_delta"] == pytest.approx(abs(data["signed_convergence_delta"]))
...
class TestLRPRules:
    ...
    def test_epsilon_rule_stabilizes(
        self, simple_classifier, sample_data, feature_names, class_names
    ):
        """Two finite epsilon settings both produce finite attributions."""
        from explainiverse.adapters import PyTorchAdapter
        from explainiverse.explainers.gradient import LRPExplainer
    ...
    def test_gamma_rule_reports_effective_configuration(
        self, simple_classifier, positive_sample_data, feature_names, class_names
    ):
        """Gamma propagation reports its exact per-layer rule and parameter."""
        from explainiverse.adapters import PyTorchAdapter
        from explainiverse.explainers.gradient import LRPExplainer
        ...
        data = explanation_gamma.explanation_data

        assert data["rule"] == "gamma"
        weighted_parameters = [
            parameters
            for parameters in data["effective_layer_parameters"].values()
            if parameters["rule"] == "gamma"
        ]
        assert weighted_parameters
        assert all(parameters["gamma"] == pytest.approx(0.25) for parameters in weighted_parameters)

    def test_alpha_beta_returns_finite_attributions(
        self, simple_classifier, sample_data, feature_names, class_names
    ):
        """Configured alpha-beta propagation returns finite attributions."""
        from explainiverse.adapters import PyTorchAdapter
        from explainiverse.explainers.gradient import LRPExplainer
    ...
    def test_z_plus_non_negative(
        ...
        assert all(
            a >= -1e-6 for a in attributions
        ), f"Unexpected negative attributions: {attributions}"

    def test_standalone_rules_report_requested_configuration(
        self, simple_classifier, sample_data, feature_names, class_names
    ):
        """Every standalone rule reports the configuration it actually applies."""
        from explainiverse.adapters import PyTorchAdapter
        from explainiverse.explainers.gradient import LRPExplainer
        ...
        instance = np.abs(sample_data[0]) + 0.1
        ...
        for rule in rules:
            ...
            explainer = LRPExplainer(
                model=adapter, feature_names=feature_names, class_names=class_names, **kwargs
            )

            explanation = explainer.explain(instance, target_class=0)
            assert explanation.explanation_data["rule"] == rule
            assert all(
                np.isfinite(value) for value in explanation.explanation_data["attributions_raw"]
            )
...
class TestLRPComposite:
    ...
    def test_composite_rule_creation(self, deep_classifier, feature_names, class_names):
        ...
        explainer = LRPExplainer(
            model=adapter, feature_names=feature_names, class_names=class_names, rule="composite"
        )
    ...
    def test_composite_rule_explain(self, deep_classifier, sample_data, feature_names, class_names):
        ...
        explanation = explainer.explain(np.abs(sample_data[0]) + 0.1)
...
class TestLRPBatch:
    ...
    def test_batch_consistent_with_individual(
        self, simple_classifier, sample_data, feature_names, class_names
    ):
...
class TestLRPLayerRelevances:
    ...
    def test_layer_relevances_available(
        self, deep_classifier, sample_data, feature_names, class_names
    ):
    ...
    def test_layer_relevance_endpoints_match_reported_values(
        self, deep_classifier, sample_data, feature_names, class_names
    ):
        """Input/output endpoint arrays match the separately reported values."""
        from explainiverse.adapters import PyTorchAdapter
        from explainiverse.explainers.gradient import LRPExplainer
        ...
        layers = result["layer_relevances"]
        np.testing.assert_allclose(layers["input"], result["input_relevances"])
        np.testing.assert_allclose(layers["output"], [result["target_output"]])
...
class TestLRPArchitectures:
    ...
    def test_model_with_batchnorm(
        self, model_with_batchnorm, sample_data, feature_names, class_names
    ):
        ...
        adapter = PyTorchAdapter(
            model_with_batchnorm, task="classification", class_names=class_names
        )
...
class TestLRPRegistry:
    ...
    def test_lrp_via_registry(self, simple_classifier, feature_names, class_names):
        ...
        explainer = default_registry.create(
            "lrp", model=adapter, feature_names=feature_names, class_names=class_names
        )
...
class TestLRPDeterminism:
    ...
    def test_lrp_deterministic_across_rules(
        self, simple_classifier, sample_data, feature_names, class_names
    ):
        ...
        for rule in ["epsilon", "gamma", "z_plus"]:
            ...
            instance = sample_data[0]
            if rule == "z_plus":
                instance = np.abs(instance) + 0.1
            exp_1 = explainer.explain(instance, target_class=0)
            exp_2 = explainer.explain(instance, target_class=0)
...
class TestLRPEdgeCases:
    ...
    def test_single_feature(self, class_names):
        ...
        explainer = LRPExplainer(
            model=adapter, feature_names=["single_feature"], class_names=class_names
        )
...
class TestLRPTypeSafety:
    ...
    def test_attributions_are_python_floats(
        self, simple_classifier, sample_data, feature_names, class_names
    ):
    ...
    def test_attributions_raw_are_python_floats(
        self, simple_classifier, sample_data, feature_names, class_names
    ):
...
class TestLRPConv2d:
    ...
    def test_lrp_conv2d_z_plus(self, simple_cnn, image_feature_names, class_names):
        ...
        explainer = LRPExplainer(
            model=adapter, feature_names=image_feature_names, class_names=class_names, rule="z_plus"
        )
    ...
    def test_lrp_maxpool2d(self, simple_cnn, image_data, image_feature_names, class_names):
        ...
        explainer = LRPExplainer(
            model=adapter, feature_names=image_feature_names, class_names=class_names
        )
    ...
    def test_lrp_cnn_with_batchnorm2d(
        self, cnn_with_batchnorm, image_data, image_feature_names, class_names
    ):
    ...
    def test_lrp_cnn_batch_processing(
        self, simple_cnn, image_data, image_feature_names, class_names
    ):
    ...
    def test_lrp_cnn_reports_requested_rules(
        self, simple_cnn, image_data, image_feature_names, class_names
    ):
        """CNN explanations report each configured weighted-layer rule."""
        from explainiverse.adapters import PyTorchAdapter
        from explainiverse.explainers.gradient import LRPExplainer

        adapter = PyTorchAdapter(simple_cnn, task="classification", class_names=class_names)

        instance = np.abs(image_data[0]) + 0.1
        for rule in ["epsilon", "gamma", "z_plus"]:
            explainer = LRPExplainer(
                model=adapter, feature_names=image_feature_names, class_names=class_names, rule=rule
            )

            explanation = explainer.explain(instance, target_class=0)
            data = explanation.explanation_data
            assert data["rule"] == rule
            weighted_rules = [data["effective_layer_rules"][index] for index in (0, 3, 7)]
            assert weighted_rules == [rule, rule, rule]
...
class TestLRPActivations:
    ...
    def test_leaky_relu(self, feature_names, class_names):
        ...
        model = nn.Sequential(nn.Linear(4, 16), nn.LeakyReLU(0.1), nn.Linear(16, 3))
        model.eval()
    ...
    def test_elu(self, feature_names, class_names):
        ...
        model = nn.Sequential(nn.Linear(4, 16), nn.ELU(), nn.Linear(16, 3))
        model.eval()
    ...
    def test_sigmoid(self, feature_names, class_names):
        ...
        model = nn.Sequential(nn.Linear(4, 16), nn.Sigmoid(), nn.Linear(16, 3))
        model.eval()
...
class TestLRPCompareRules:
    ...
    def test_compare_rules_returns_all(
        self, simple_classifier, sample_data, feature_names, class_names
    ):
    ...
    def test_compare_rules_contains_attributions(
        self, simple_classifier, sample_data, feature_names, class_names
    ):
    ...
    def test_compare_rules_subset(self, simple_classifier, sample_data, feature_names, class_names):
        ...
        comparison = explainer.compare_rules(sample_data[0], rules=["epsilon", "gamma"])
...
class TestLRPFlatten:
    ...
    def test_flatten_in_network(self, class_names):
        ...
        explainer = LRPExplainer(
            model=adapter, feature_names=[f"f_{i}" for i in range(4)], class_names=class_names
        )
