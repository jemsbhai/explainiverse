...
- Security commit `9ad738a2dc62777ae199b03a3348b24b45006da2` remains in open, unmerged PR #4; its parent
  ...
  CUDA reporters remained red. Merging still requires a genuine owner dispatch on approved
  capacity at that exact future head, not a gate waiver.
...
| B04 | **BLOCKED — GPU quota denied; no hardware evidence** | Repository administrator, GCP quota authority, and GPU-infrastructure owner | Obtain at least two approved GPUs in an isolated Linux deployment; coordinate restoration and temporarily remove every non-owner collaborator, require zero invitations, audit installed App/automation authority, and clear custom-label queues. Set exact variables and owner-dispatch the reviewed ref with no runner online; verify the exact queued jobs, then register fresh one-job JIT runners. | All four minimum/latest one-/two-GPU jobs complete successfully exactly once on the candidate, with the expected custom label, exact declared visible-device count, exact expected 15-node manifest, 15 executed, zero skips, retained human/App authority, queue, job, runner, and infrastructure-isolation evidence. The authority lock remains through publication. Success/failure cleanup proves variables, queues, runners, VMs, and disks are zero before the exact prior permission is re-invited and its acceptance recorded. | Dedicated billed project `explainiverse-release-ci-26`/`305968033598` has an isolated custom VPC, disabled default service account, and zero VMs/runners/Actions variables. Global GPU, standard T4, and preemptible T4/L4 requests were denied; global quota remains 0 and regional quotas remain 1, insufficient for the two-GPU lane. `b-urge` remains `write` while no execution surface exists; installed-App authority is unaudited at the authentication boundary. PR #4's workflow guard is defense in depth; no runner registered, no GPU job ran, and no hardware claim is made. |
