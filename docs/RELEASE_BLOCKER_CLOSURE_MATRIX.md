...
continuation described below. A repository guard can make future evidence falsifiable, but it
...
landed, and every selected non-CUDA `push` context succeeded on current `main`, but no approved
one-/two-GPU capacity exists. That external B04 failure prevents the trusted CUDA dispatch,
security PR #4's merge, a final-candidate zero-violation B02 capture, and every downstream
publication, governance, and recovery action. No tag, PyPI publication, GitHub Release, release
governance record, or staged recovery drill has been created, and no manual accessibility or GPU
...
- Live `main` protection is now strict and requires all 23 reviewed contexts, each provider-bound
  ...
  `success`. Thus 21/23 provider-bound exact-`main` checks are green; B02 settings are configured,
  but B02 acceptance is not.
...
- Security commit `9ad738a2dc62777ae199b03a3348b24b45006da2` remains in open, unmerged PR #4; its parent
  is `a9789d009f6ec5134bc53b9d2f6a8b59726e75c7`. It hardens the public-repository boundary so
  `pull_request` and `push` events cannot route untrusted code onto custom self-hosted GPU runners;
  only an authenticated owner `workflow_dispatch` can produce the required CUDA contexts. Its PR
  event therefore fails those required CUDA contexts by design. Merging it requires a genuine
  owner dispatch on approved capacity at that exact head, not a gate waiver.
...
| B02 | **BLOCKED — controls configured; two CUDA checks fail** | GitHub repository administrator and CI/merge authority | Keep all 23 policy contexts provider-bound to GitHub Actions app ID 15368 and immutable Releases enabled; obtain genuine CUDA success on the final candidate and repeat the authenticated capture. | A capture no older than 30 minutes has `repository_controls_accepted=true`, zero violations, immutable Releases enabled, exact policy/snapshot digests, exact context/app bindings, and one successful provider-bound check per required name on the candidate SHA; actor and triggering actor both equal the capture principal, and the retained run attempt/triggering actor match the Actions API source run. | Strict `main` protection now has all 23 exact contexts with app ID `15368`, and immutable Releases is enabled. Capture `4833e9f2…1b34e5` at `2026-08-11T18:39:00.267775+00:00` reduced the predecessor's 19 violations to exactly two: CUDA minimum/latest completed with failure. All other 21 exact-`main` checks succeeded. The capture remains rejected, and no gate is waived. |
| B03 | **BLOCKED — recovery drill not executed** | Separately authorized release operator | On the eventual build/attest/OIDC run, request the deliberate post-PyPI failure, then dispatch recovery from the immutable release tag and recover only the downstream GitHub Release from that original run. Do not reuse 0.14.0. | Source run concludes failure; build, attest, and publish each have exactly one successful attempt; the release job fails at the explicit drill step with later release steps skipped. Recovery executes from `refs/tags/<tag>` at that tag's exact commit, verifies attestations, and proves original, PyPI, and GitHub files byte-identical without a second upload. Retain source/all-attempt jobs, inventories, hashes, service JSON, and recovery evidence. | No tag, PyPI upload, source publication run, or recovery run exists. The drill is downstream of B04, the final B02 capture, and B01 configuration; legacy 0.14.0 cannot satisfy it. |
| B04 | **BLOCKED — GPU quota denied; no hardware evidence** | Repository administrator, GCP quota authority, and GPU-infrastructure owner | Obtain at least two approved GPUs in an isolated Linux runner deployment carrying the exact labels `explainiverse-cuda-single` and `explainiverse-cuda-two`, set `CUDA_SINGLE_RUNNER` and `CUDA_TWO_RUNNER`, and owner-dispatch `cuda-ci.yml` on PR #4 and then final `main`. | All four minimum/latest one-/two-GPU jobs complete successfully exactly once on the candidate, with the expected custom label, exact declared visible-device count, exact expected 15-node manifest, 15 executed, zero skips, and retained job/runner evidence bound into preflight. The infrastructure owner separately records isolation approval. | Dedicated billed project `explainiverse-release-ci-26`/`305968033598` has an isolated custom VPC, disabled default service account, and zero VMs/runners/Actions variables. Global GPU, standard T4, and preemptible T4/L4 increase requests were denied; global quota remains 0 and the requested regional accelerator quotas remain 1, insufficient for the two-GPU lane. The exact dedicated and unused-fallback denial traces are archived above. PR #4 correctly refuses PR/push self-hosted routing. No runner registered, no GPU job ran, and no hardware claim is made. |
...
The operative cascade is B04 quota/capacity → trusted CUDA evidence on PR #4 → merge of the
self-hosted-runner security boundary → final-main reruns → zero-violation B02 capture →
authenticated B01 publisher/signing configuration and sole OIDC publication → B08 release record
and B03 downstream-only recovery. Current-main and PR #4 non-CUDA successes are valuable direct
evidence, but they cannot bypass that order or turn `a9789d0…` into the final candidate. The stable
release recommendation therefore remains **NO**.
